...
if ( !text ) return citel.reply( `*֎╎رد عـلـي رسـالـه شـخـص واكـتـب اسـم مـثـال┇.كونكت Zenon*`)
...
const vcard = 'BEGIN:VCARD\n' +
 ...
 cmd({
         ...
         async(Void, citel, text) => {
             ...
             if (text) {
                 anu = text.split("|");
                 pack = anu[0] !== "" ? anu[0] : citel.pushName + 'Zenon 𝐵𝑂𝑇-𝑀𝐷';
...
const { Sticker, createSticker, StickerTypes } = require("wa-sticker-formatter");
  ...
  const response = await fetch(`https://tenor.googleapis.com/V1/featured?key=AIzaSyAyimkuYQYF_FXVALexPuGQctUWRURdCYQ&contentfilter=high&media_filter=png_transparent&component=proactive&collection=emoji_kitchen_v5&q=${emoji1}_${emoji2}`);
